...
void validate_data_compare(uint64_t should_not_be_smaller,
                       uint64_t should_not_be_greater, const string& mess) {
...
static bool in_range(int to_test, int upper_limit) {
...
void Client_bomberman::update_player_scores() {
...
void Client_bomberman::update_bomb_timers() {
    for (auto & bomb : bombs) {
        Bomb& bomb_data = bomb.second;
        if (bomb_data.placed_now) {
            bomb_data.placed_now = false;
        }
        else {
            bomb_data.timer--;
        }
 ...
 *      has been marked as destoryed by the server and added to explosion
 *      points in a different function
 */
bool Client_bomberman::check_if_possible_to_propagate(int potential_x,
                                                      int potential_y) {
 ...
 */
void Client_bomberman::add_explosion_cross(bomb_id_dt exploding_bomb_id) {
 ...
 *  GUI -> client -> server
 */
bool Client_bomberman::is_valid_gui_message(size_t read_bytes) {
...
void Client_bomberman::process_data_from_gui(
        const boost::system::error_code& error,
                           std::size_t read_bytes) {
    if ((!error || error == boost::asio::error::eof)
        && is_valid_gui_message(read_bytes)) {
            ...
            if (!gameStarted) {
                ...
                data_to_send_server[bytes_to_send] =
                        (char)player_name.length();
                bytes_to_send++;

                strcpy(data_to_send_server + bytes_to_send,
                       player_name.c_str());
                ...
                switch (message_type) {
                    case (InputMessage::PlaceBombGUI):
                        validate_data_compare(read_bytes, 1,
                                      "Incorrect PlaceBomb gui message\n");
                    ...
                    case (InputMessage::PlaceBlockGUI):
                        validate_data_compare(read_bytes, 1,
                                      "Incorrect PlaceBlock gui message\n");
                    ...
                    case (InputMessage::MoveGUI):
                        ...
                        data_to_send_server[bytes_to_send] =
                                (char)received_data_gui[bytes_to_send];
                        bytes_to_send++;
        ...
        boost::asio::async_write(socket_tcp_,
             boost::asio::buffer(data_to_send_server,
                                 bytes_to_send),
             boost::bind(
...
void Client_bomberman::after_sent_data_to_server(
...
void Client_bomberman::receive_from_gui_send_to_server() {
    // Receive from GUI
 ...
 *  Server -> client -> GUI
 */
void Client_bomberman::receive_from_server_send_to_gui() {
...
void Client_bomberman::check_message_id(const boost::system::error_code& error,
                               std::size_t read_bytes) {
    if (!error || error == boost::asio::error::eof) {
 ...
 *  Hello
 */
void Client_bomberman::read_server_name_length (
                                const boost::system::error_code& error,
                                std::size_t read_bytes) {
    if (!error || error == boost::asio::error::eof) {
        validate_data_compare(read_bytes, 1,
                              "Hello: incorrect server name length\n");
...
void Client_bomberman::read_hello_body_without_string (
        const boost::system::error_code& error,
                             std::size_t read_bytes) {
    if (!error || error == boost::asio::error::eof) {
 ...
 * Accepted player
 */
void Client_bomberman::read_player_id_and_name_length(
                                    const boost::system::error_code& error,
                                    std::size_t read_bytes,
                                    int num_repetitions,
    ...
    if (!error || error == boost::asio::error::eof || read_bytes > 0) {
        if (num_repetitions > 0) {
            ...
            boost::asio::async_read(socket_tcp_,
                ...
                boost::bind(
                    &Client_bomberman::read_player_name_string_and_address_length,
                    this,
                    boost::asio::placeholders::error,
                    boost::asio::placeholders::bytes_transferred,
    ...
    if (!error || error == boost::asio::error::eof || read_bytes > 0) {
        ...
        boost::asio::async_read(socket_tcp_,
            boost::asio::buffer(received_data_server,
                                address_length),
            boost::bind(
                    &Client_bomberman::read_player_address_string_add_player,
                    this,
                    boost::asio::placeholders::error,
                    boost::asio::placeholders::bytes_transferred,
...
void Client_bomberman::read_player_address_string_add_player(
    ...
    if (!error || error == boost::asio::error::eof || read_bytes > 0) {
        ...
        players.insert(make_pair(player_id, Player{player_name_temp,
                                                   player_address}));
        scores.insert(make_pair(player_id, 0));
...
void Client_bomberman::read_player_map_length (
    ...
    if (!error || error == boost::asio::error::eof || read_bytes > 0) {
        ...
        size_t map_length = big_to_native(*(map_list_length_dt *)
 ...
 */
void Client_bomberman::read_event_id (const boost::system::error_code& error,
...
* Bomb placed
*/
void Client_bomberman::read_bomb_id_and_position (
                    const boost::system::error_code& error,
                    std::size_t read_bytes, map_list_length_dt
                    num_repetitions) {
    if (!error || error == boost::asio::error::eof || read_bytes > 0) {
 ...
 *  Bomb exploded
 */
void Client_bomberman::read_bomb_id_and_player_id_length (
                                        const boost::system::error_code& error,
                                        std::size_t read_bytes,
                                        map_list_length_dt
                                        num_repetitions) {
 ...
 */
void Client_bomberman::read_player_id (const boost::system::error_code& error,
                     std::size_t read_bytes, map_list_length_dt
                     num_repetitions,
                     map_list_length_dt player_id_left_elements,
                     bomb_id_dt bomb_id) {
    if (!error || error == boost::asio::error::eof) {
        validate_data_compare(read_bytes, player_id_bytes,
                              "Error in player id");
 ...
 */
void Client_bomberman::read_blocks_destroyed_length (
                                    const boost::system::error_code& error,
                                    std::size_t read_bytes,
                                    map_list_length_dt
                                    num_repetitions,
    ...
    if (!error || error == boost::asio::error::eof) {
        validate_data_compare(read_bytes, map_list_length,
                              "Bomb exploded: Incorrect blocks "
                              "destroyed length");
...
void Client_bomberman::read_blocks_destroyed_positions (const boost::system::error_code& error,
                          std::size_t read_bytes,
                          map_list_length_dt
                          num_repetitions,
 ...
 *  Player moved
 */
void Client_bomberman::read_player_id_and_position (const boost::system::error_code& error,
                                  std::size_t read_bytes, map_list_length_dt
                                  num_repetitions) {
    if (!error || error == boost::asio::error::eof) {
        ...
        player_id_dt player_id = big_to_native(*(player_id_dt *)
                                received_data_server);
        position_dt x = big_to_native(*(position_dt*)
 ...
 *  Block placed
 */
void Client_bomberman::read_block_position (const boost::system::error_code& error,
                          std::size_t read_bytes, map_list_length_dt
                          num_repetitions) {
    if (!error || error == boost::asio::error::eof) {
        validate_data_compare(read_bytes, position_bytes,
 ...
 *  Game ended
 */
void Client_bomberman::read_score_map_length (const boost::system::error_code& error,
                            std::size_t read_bytes) {
    if (!error || error == boost::asio::error::eof) {
        validate_data_compare(read_bytes, map_list_length,
...
void Client_bomberman::read_player_id_score (const boost::system::error_code& error,
                          std::size_t read_bytes, map_list_length_dt
                          num_repetitions) {
    if (!error || error == boost::asio::error::eof) {
...
void Client_bomberman::write_map_to_buffer(size_t& bytes_to_send, int map_type) {
    ...
    *(uint32_t *) (data_to_send_gui +
    ...
    if (map_type == map_players) {
        for (auto &player: players) {
            ...
            // Player address
            data_to_send_gui[bytes_to_send] =
                                    (char) player_data.address.length();
            bytes_to_send++;
        ...
        for (auto & player_position : player_positions) {
            ...
            *(uint16_t *) (data_to_send_gui + bytes_to_send) =
                                   (native_to_big(current_position.x));
            bytes_to_send += single_position_bytes;

            *(uint16_t *) (data_to_send_gui + bytes_to_send) =
                                   (native_to_big(current_position.y));
            bytes_to_send += single_position_bytes;
        }
    }
    else if (map_type == map_score) {
        for (auto & player_score : scores) {
            ...
            *(score_dt *)(data_to_send_gui + bytes_to_send) =
                                    native_to_big(player_score.second);
            bytes_to_send += score_bytes;
        }
    }
}

void Client_bomberman::write_list_to_buffer(size_t& bytes_to_send, int list_type) {
    ...
    *(uint32_t *) (data_to_send_gui + bytes_to_send) =
                                (native_to_big((uint32_t)length_to_send));
    bytes_to_send += map_list_length;
...
size_t Client_bomberman::create_udp_message(bool is_Lobby) {
    size_t bytes_to_send = 0;
    ...
    *(uint16_t*)(data_to_send_gui + bytes_to_send) =
                                    (native_to_big(game_status.size_x));
    bytes_to_send += lobby_exc_pc_bytes;

    *(uint16_t*)(data_to_send_gui + bytes_to_send) =
                                    (native_to_big(game_status.size_y));
    bytes_to_send += lobby_exc_pc_bytes;

    *(uint16_t*)(data_to_send_gui + bytes_to_send) =
                                    (native_to_big(game_status.game_length));
    bytes_to_send += lobby_exc_pc_bytes;

    if (!is_Lobby) {
        *(uint16_t *) (data_to_send_gui + bytes_to_send) =
                                    (native_to_big(game_status.turn));
        bytes_to_send += lobby_exc_pc_bytes;
    }
    else {
        ...
        *(uint16_t *) (data_to_send_gui + bytes_to_send) =
                                    (native_to_big(game_status.bomb_timer));
        bytes_to_send += lobby_exc_pc_bytes;
    }
...
void Client_bomberman::process_data_from_server_send_to_gui() {
...
void Client_bomberman::after_send_to_gui([[maybe_unused]] const boost::system::error_code& error,
                       [[maybe_unused]] std::size_t sent_bytes) {
    // Repeat the cycle, reset variables
...
Client_bomberman::Client_bomberman(io_context& io, const string& server_name,
...
{
    /*
     * Functions responsible for connection throw exceptions
     * in case of failure. The documentation doesn't specify the specific
     * return values.
     */
    try {
        ...
        udp::resolver::results_type gui_endpoints_to_send_ =
                            udp_resolver_.resolve(gui_name, gui_port);
